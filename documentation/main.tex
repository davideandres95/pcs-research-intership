% Load the institute for communications engineering document class
\documentclass[chair=lnt]{ICEthesis} % options for the chair are lnt (default), lut or cod

\input{utils/packages}
\input{utils/misc}
\input{utils/acronyms}
\usepackage{physics}
\usepackage{float}
\usepackage{tikz}
\usepackage{pgfplots}
\usepackage{caption}
\usepackage[backend=bibtex,style=ieee-alphabetic,natbib=true, maxcitenames=2]{biblatex} %added
\addbibresource{refs.bib}
\usetikzlibrary{arrows}
\usetikzlibrary{shapes.arrows}
\usetikzlibrary{fit}
\newtheorem{theorem}{Theorem}

\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}

\makeglossaries

\begin{document}
% ##########
% Title page
% ##########
\ICEtitle{Research Internship} % Thesis type (Master's Thesis or Bachelor's Thesis)
    {Geometric and Probabilistic Constellation Shaping with Autoencoders}
    {David de Andrés Hernández}
    {M.Sc. Francesca Diedolo}
    {München, October 2022}
    { % your data
        David de Andrés Hernández\\
        Boschetsriederstr. 55A\\
        81379 München\\
        deandres.hernandez@tum.de
    }

\ICErecht{David de Andrés Hernández}{München, 26.10.2022}

\cleardoubleemptypage % start new double page / neue Doppelseite



% ########################################
% Table of Contents / Inhaltsverzeichnis
% ########################################
% roman page numbering, starting with page number 1
\setcounter{page}{1}
\pagenumbering{roman}

\tableofcontents
% list of figures and list of tables is optional, feel free to comment if you like
%\listoffigures
%\listoftables
\cleardoubleemptypage


% ########
% Chapters
% ########
% arabic page numbering, starting with page number 1
\setcounter{page}{1}
\pagenumbering{arabic}

% Include chapters
\input{chapters/0_abstract}
\input{chapters/1_introduction}
\input{chapters/2_preliminaries}
\input{chapters/3_contribution}
\input{chapters/4_conclusions}

\cleardoubleemptypage



% ########
% Appendix
% ########
%\appendix
%\input{chapters/appendixA}
%\input{chapters/appendixB}

% ### list of acronyms (comment out if does not work and you don't want to make it work)
\printglossary[
    type = acronym,
    title = List of Abbreviations,
    toctitle = List of Abbreviations,
    style = long,
    nonumberlist,
    numberedsection = false,
]

% ### bibliography
% Use IEEE DIN 1505 style for bibliography
%\bibliographystyle{IEEEtranSA}
%\nocite{*} % include all references without checking (don't do this)
%\bibliography{refs}
\printbibliography
\end{document}

%
% EOF!
%
%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
